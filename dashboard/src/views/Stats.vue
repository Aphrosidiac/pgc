...
const chartOptions = {
  ...
  scales: {
    x: { grid: { color: '#2a2e3f' }, ticks: { color: '#8b92a8' } },
...
<template>
  <div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
      <div v-for="c in cards" :key="c.key" class="bg-surface border border-border rounded-xl p-4">
        <p class="text-xs text-text-muted uppercase">{{ c.label }}</p>
        <p class="text-2xl font-semibold mt-1" :class="c.color">{{ stats[c.key] }}</p>
      </div>
    </div>
